function [phi, r] = solver_CahnHilliard_2D_DCT_SAV_1st(pde,domain,Nx,Ny,time,option)
...
phi_prev = pde.initphi(xx,yy);

r_prev = fun_r_init(phi_prev);
...
if 1 == option.saveflag
    ...
    ss = [dir_data '/phi1_t=' num2str(t0) '.txt'];
    fid1 = fopen(ss, 'wt');
    fprintf(fid1, '%f\n', phi_prev(:));
    fclose(fid1);
end
...
if 1 == option.plotflag
    showsolution_2D(nfigure,xx,yy,phi_prev,t);
end
...
for nt = 1:nplot
    ...
    rhs = phi_prev / (M*dt) + diff_lap(H).*g1 + tmp;
...
if 1 == option.savefinal
    ...
    filename=[name '_DCT_SAV_1st.mat'];
