function [phi, r] = solver_CahnHilliard_2D_DCT_SAV_CN(pde,domain,Nx,Ny,time,option)
...
global dt xix xiy xixx xiyy xi2 xi4 hx hy Lx Ly ...
    epsilon M C0 S
...
x_c  = domain.left   + 0.5 * hx + hx*(0:Nx-1);
y_c  = domain.bottom + 0.5 * hy + hy*(0:Ny-1);
...
[xixx, xiyy] = ndgrid(xi2x,xi2y);
xi2 = -(xixx + xiyy);
xi4 = xi2.^2;

[xix,  xiy] = ndgrid(xi_x,xi_y);
...
[phi_prev,r_prev] = solver_CahnHilliard_2D_DCT_SAV_1st(pde,domain,Nx,Ny,time1,option1);

for nt = 2:nplot
    t = t0 + nt*dt;

    phi_star = (3*phi_prev - phi_old)/2;
    
    H = fun_H(phi_star);
    
    % Step 1 
    if isfield(pde,'exactphi') && isfield(pde,'rhsphi')
        tmp = pde.rhsphi(xx,yy,t - dt/2) / M;
        % fprintf('aaa\n');
    else
        tmp = 0;
    end

    g1 = r_prev - 1/2 * inner_int(H,phi_prev);
    rhs = phi_prev / (M*dt) + diff_lap(-epsilon^2/2*diff_lap(phi_prev) + S/2*phi_prev)...
        + 1/2*diff_lap(H).*(r_prev + g1) + tmp;

    % Step 2 
    psi_B = inv_A(diff_lap(H));
    psi_C = inv_A(rhs);

    gamma =  -1/4 * inner_int(H,psi_B);
    Hphi = inner_int(H,psi_C) / (1 + gamma);

    % Step 3
    phi = 1/4 * Hphi .* psi_B + psi_C;
...
global hx hy C0
E1 = F(phi);
if sum(E1(:))*hx*hy + C0 < 0
    fprintf("Root < 0, E + C=%f\n",sum(E1(:))*hx*hy + C0);
    return;
end
r  = sqrt(sum(E1(:))*hx*hy + C0);
end
...
r = fun(phi)./sqrt(sum(E1(:))*hx*hy + C0);
end

function result = inv_A(phi)
global dt xi2 M epsilon S
    L = -epsilon^2*xi2 + S;
    phihat = dct2(phi);
    r      = phihat./(1/(M*dt) - xi2 .* L/2);
    result = idct2(r);
end
