function [phi, r] = solver_AllenCahn_2D_SAV_1st(pde,domain,Nx,Ny,time,option)
% Allen-Cahn equation
% Reference:
%
%
...
%figure(1); clf;
%contourf(x_c,y_c,real(phi_prev'),[0,0]); axis image 
%axis([x_c(1) x_c(Nx) y_c(1) y_c(Ny)])
%pause(0.01)
...
for nt = 1:nplot
    ...
    r = 1/2*Hphi + g1;
     ...
     %if mod(nt, nsave) == 0
      %  figure(1); clf;
      %  contourf(x_c, y_c, real(phi'), [0 0]);
      %  axis image;
      %  axis([x_c(1) x_c(Nx) y_c(1) y_c(Ny)]);
      %  title(sprintf('AC Equation - Iteration %d, Time = %.4f', nt, nt*dt));
      %  drawnow;
    %end
